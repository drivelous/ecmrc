...
class OrderModify(object):
	"""Helper class to modify orders every time user changes account details"""
	...
	def use_shipping(self):
		"""Takes UseAddressForm cleaned_data and uses specified
		address ID as order's shipping address
		"""
	...
	def save_shipping(self):
		"""Called when user adds new shipping address during checkout.
		Assumes user wants to use address for order after created
		"""
	...
	def delete_card(self, customer, card):
		"""Deletes card from user's account and Stripe profile and manages
		the ramifications of reassigning new default card"""
	...
	def use_card(self, card):
		"""Ties user's previously added card to the current open order"""
